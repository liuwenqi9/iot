...
public class HttpKit {
	...
	public static String httpClientPost(String url, String data, boolean isHttps) throws ParseException, IOException {
		...
		HttpPost httppost = new HttpPost(url);
		// 建立HttpPost对象
		StringEntity myEntity = new StringEntity(data, Constant.DEFAULT_CHAR_SET);
		...
		if (response.getStatusLine().getStatusCode() == 200) {// 如果状态码为200,就是正常返回
			HttpEntity httpEntity = response.getEntity();
			result = EntityUtils.toString(httpEntity, Constant.DEFAULT_CHAR_SET);
	 ...
	 * @throws IOException
	 */
	public static String httpClinetGetForMqAPI(String ip, int port, String username, String password, String queueName, Boolean isHttps) throws ClientProtocolException, IOException {
		CredentialsProvider credsProvider = new BasicCredentialsProvider();
		credsProvider.setCredentials(new AuthScope(ip, port + 10000), new UsernamePasswordCredentials(username, password));
		CloseableHttpClient httpClient = null;
		if (isHttps) {
			httpClient = HttpClients.custom().setSSLSocketFactory(createSSLConnSocketFactory()).build();
		} else {
		...
		String result = null;
		String url = isHttps ? "https" : "http" + "://" + ip + ":1" + port + "/api/queues/%2F/" + queueName;
