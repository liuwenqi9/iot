...
<script>
	...
	function area_add() {
		openDialog({
			...
			area : [ '650px', '420px' ],
			scrollbar : false,
			content : "devicesarea/cameraAddPage.do?devicesId="+DevicesId,
			viewid : "dialogPage3",
			btn : [ '确定', '取消' ],
			yes : function(index) {
				var boo = validate_devicesareaform();
				if(boo){
				var devicesareaid = $("#areacodeid").find("option:selected").val();
				var lefttopx = $("#lefttopx").val().trim();
				var lefttopy = $("#lefttopy").val().trim();
				var rightbottomx = $("#rightbottomx").val().trim();
				var rightbottomy = $("#rightbottomy").val().trim();
				formOpt($('#devicesareaform'), "devicesarea/cameraAdd.do?devicetypecode=002&deviceid="+DevicesId+"&devicesareaid="+devicesareaid+"&lefttopx="+lefttopx+"&lefttopy="+lefttopy+"&rightbottomx="+rightbottomx+"&rightbottomy="+rightbottomy, devicesarea_table, index, null);
				} 
			}
		});
	}
